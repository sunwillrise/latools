...
class D(object):
    ...
    @_log
    def filter_expand(self, start=0, end=0, filt=True):
        """
        Remove points from the start and end of filter regions.
        ...
        f = self.filt.grab_filt(filt)
        nf = filters.expand(f, start, end)
        
        self.filt.add('expanded_filter',
                    nf,
                    'Expanded Filter ({:.0f} start, {:.0f} end)'.format(start, end),
                    params, setn=self.filt.maxset + 1)
