...
class analyse(object):
    ...
    def duplicate_files(self, samples=None, newfnames=None, double = True):
        ...
        for i, s in enumerate(samples):
            if newfnames == None:
                ...
                nf1 = newfnames[i] + 'with-inc.csv'
                nf2 = newfnames[i] + 'excl-inc.csv'
                nf3 = newfnames[i] + '.csv'
            if double:
                shutil.copy(self.data[s].file, 
                            self.folder + '/'+ nf1)
                newfiles = np.append(newfiles, nf1)
                shutil.copy(self.data[s].file, 
                        self.folder + '/' + nf2)
    ...
    def select_range_plot(self, sample, analyte, s=None, e=None):
        self.data[sample].filter_select_range(startIndex=s, endIndex=e)
        self.filter_on(samples=sample, filt=str(x))
        self.trace_plots(samples=sample, analytes = analyte, ranges=True, filt=True)
        self.filter_off(samples=sample, filt=str(x))
        return
